...
def make_parser():
    ...
    parser.add_argument(
        ...
        default=True,
        type=bool,
        help='save frames from video as individual frames'
    )
...
class Predictor(object):
    ...
    def visual(self, frame_idx, output, img_info, cls_conf=0.35):
        ratio = img_info["ratio"]
        img = img_info["raw_img"]
        if output is None:
            return img, {'frame_idx': frame_idx, 'data': []}
        output = output.cpu()
...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    if args.save_imgs:
        save_dir = os.path.join(save_folder, 'rgb')
        ...
        while True:
            ...
            if frame_idx % (args.skip + 1) == 0:
                if ret_val:
                    ...
                    logger.info('Frame {:} / {:}, Inference time: {:.4f}s'.format(frame_idx, int(cap.get(cv2.CAP_PROP_FRAME_COUNT)), t))
...
def images_demo(predictor, vis_folder, current_time, args):
    save_folder = os.path.join(
        vis_folder, time.strftime("%Y_%m_%d_%H_%M_%S", current_time)
    )
    os.makedirs(save_folder, exist_ok=True)
...
def main(exp, args):
    ...
    if args.demo == "image":
        ...
        images_demo(predictor, vis_folder, current_time, args)
